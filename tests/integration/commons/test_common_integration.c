...
static int test_full_workflow(void) {
    ...
    airy_error_t err = AIRY_ERR_INVALID_PARAM;
...
static int test_error_handling_chain(void) {
    ...
    /* 模拟错误�?*/
    airy_error_chain_add(chain, AIRY_ERR_INVALID_PARAM, "file1.c", 10, "func1", "First error");
    airy_error_chain_add(chain, AIRY_ERR_OUT_OF_MEMORY, "file2.c", 20, "func2", "Second error");
    
    int count = airy_error_chain_count(chain);
