...
{
    (void)arg;
    void* p = airy_mem_alloc(1024 * 1024);
    if (p) airy_mem_free(p);
}
...
{
    (void)arg;
    void* p = airy_mem_alloc_ex(256, __FILE__, __LINE__);
    if (p) airy_mem_free(p);
}
...
{
    (void)arg;
    void* p = airy_mem_aligned_alloc(512, 64);
    if (p) airy_mem_aligned_free(p);
}
...
{
    (void)arg;
    airy_metric_counter_inc("bm_counter", "tag1", 1.0);
}
...
static void bm_trace_span_start(void* arg)
{
    ...
    airy_trace_span_start(&ctx, "bm_service", "bm_op");
}
...
static void test_benchmark_observability(void)
{
    ...
    airy_observability_config_t cfg = {
        .enable_metrics = 1, .enable_tracing = 1, .enable_health_check = 1
    };
    airy_observability_init(&cfg);
...
static void bm_sys_memory_write(void* arg)
{
    (void)arg;
    char* record_id = NULL;
    airy_sys_memory_write("bm_data", 7, "{\"type\":\"bm\"}", &record_id);
...
static void* bm_concurrent_thread_fn(void* arg)
{
    ...
    for (int i = 0; i < 10000; i++) {
        void* p = airy_mem_alloc(128 + (size_t)(ctx->thread_id % 8) * 16);
        if (p) {
            AIRY_MEMSET(p, (unsigned char)(ctx->thread_id + i), 128 + (size_t)(ctx->thread_id % 8) * 16);
            airy_mem_free(p);
            ctx->alloc_count++;
        }
    }
...
static void test_benchmark_concurrent(void)
{
    ...
    airy_thread_t threads[BM_THREAD_COUNT];
    ...
    for (int i = 0; i < BM_THREAD_COUNT; i++) {
        ...
        airy_platform_thread_create(&threads[i], bm_concurrent_thread_fn, &ctxs[i]);
    }
