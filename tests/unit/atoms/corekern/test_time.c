...
int test_time_monotonic() {
    ...
    // 测试单调时间（毫秒）
    uint64_t start_ms = airy_time_monotonic_ms();
    if (start_ms == 0) {
    ...
    // 验证时间递增
    uint64_t end_ns = airy_time_monotonic_ns();
    if (end_ns < start_ns) {
        printf("Monotonic time (ns) did not increase\n");
        return 1;
    }

    uint64_t end_ms = airy_time_monotonic_ms();
    if (end_ms < start_ms) {
...
int test_time_current() {
    ...
    // 测试当前时间（纳秒）
    uint64_t start_ns = airy_time_current_ns();
    if (start_ns == 0) {
    ...
    // 测试当前时间（毫秒）
    uint64_t start_ms = airy_time_current_ms();
    if (start_ms == 0) {
    ...
    uint64_t realtime_ns = airy_time_realtime_ns();
    if (realtime_ns == 0) {
...
int test_time_sleep() {
    ...
    // 测试纳秒睡眠
    uint64_t start = airy_time_monotonic_ns();
    airy_error_t err = airy_time_nanosleep(100000000ULL); // 100ms
    if (err != AIRY_SUCCESS) {
        printf("Failed to sleep: %d\n", err);
        return 1;
    }
    uint64_t end = airy_time_monotonic_ns();
...
int test_timer() {
    ...
    // 创建定时�?
    airy_timer_t* timer = airy_timer_create(timer_callback, NULL);
    if (!timer) {
    ...
    // 测试启动定时�?
    airy_error_t err = airy_timer_start(timer, 100, 1); // 100ms, one-shot
    if (err != AIRY_SUCCESS) {
...
int test_event() {
    ...
    // 创建事件
    airy_event_t* event = airy_event_create();
    if (!event) {
    ...
    // 测试等待事件（超时）
    airy_error_t err = airy_event_wait(event, 100);
    if (err != AIRY_ETIMEDOUT) {
        printf("Event wait should have timed out: %d\n", err);
        airy_event_destroy(event);
        return 1;
    }

    // 测试触发事件
    err = airy_event_signal(event);
    if (err != AIRY_SUCCESS) {
