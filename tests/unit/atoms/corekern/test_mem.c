...
int test_mem_alloc_free() {
    ...
    // 测试对齐内存分配
    void* aligned_ptr = airy_mem_aligned_alloc(1024, 16);
    if (!aligned_ptr) {
    ...
    if ((uintptr_t)aligned_ptr % 16 != 0) {
        ...
        airy_mem_free(aligned_ptr);
        return 1;
    }

    // 测试内存写入
    AIRY_MEMSET(aligned_ptr, 0xBB, 1024);

    // 测试内存释放
    airy_mem_free(aligned_ptr);

    // 测试内存重新分配
    void* realloc_ptr = airy_mem_alloc(512);
    if (!realloc_ptr) {
    ...
    // 写入数据
    AIRY_MEMSET(realloc_ptr, 0xCC, 512);

    // 重新分配
    void* new_ptr = airy_mem_realloc(realloc_ptr, 1024);
    if (!new_ptr) {
        printf("Failed to reallocate memory\n");
        airy_mem_free(realloc_ptr);
        return 1;
    }
