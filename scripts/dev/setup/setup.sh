...
install_dependencies() {
    ...
    if [ "$OS" = "linux" ]; then
        if command -v vcpkg &> /dev/null; then
            airy_log_info "vcpkg found: $(vcpkg version | head -1)"
        else
            airy_log_warn "vcpkg not found in PATH"
            airy_log_info "Please install vcpkg and set VCPKG_ROOT"
            airy_log_info "See: https://github.com/microsoft/vcpkg"
        fi
        
        if command -v python3 &> /dev/null; then
            airy_log_info "Python3: $(python3 --version)"
        else
            airy_log_error "Python3 not found"
        fi
        
    elif [ "$OS" = "macos" ]; then
        if command -v brew &> /dev/null; then
            airy_log_info "Homebrew installed"
        else
            airy_log_warn "Homebrew not found"
        fi
        ...
        airy_log_error "Windows not supported by this script"
        airy_log_info "Please use setup.ps1 on Windows"
        return 1
    fi
    
    airy_log_info "Dependencies check complete"
}
...
build_project() {
    ...
    if [ ! -f "$BUILD_SCRIPT" ]; then
        airy_log_error "Build script not found: $BUILD_SCRIPT"
        airy_log_info "Available scripts in pipeline/build/:"
        ls -la "${SCRIPT_DIR}/../../ci/pipeline/build/" 2>/dev/null || airy_log_warn "Cannot list pipeline/build directory"
        return 1
    fi
...
full_setup() {
    airy_log_info "Starting full setup..."
...
main() {
    ...
    if [ $# -eq 0 ]; then
        ...
        case $choice in
            ...
            4) full_setup ;;
            5|q|Q) 
                airy_log_info "Exiting..."
                exit 0
                ;;
            *) 
                airy_log_error "Invalid option: $choice"
                exit 1
                ;;
        esac
    else
        case "${1:-}" in
            ...
            --help|-h|help)
                ...
                ;;
            *)
                airy_log_error "Unknown option: $1"
                show_menu
                exit 1
                ;;
        esac
